...
## Personal's note (Erwan)
This template has been modified to fit my needs.

First, the training json were generated using another tool so they are not JSONL.

To convert them into spacy, a new script file was created: scripts/binary_converter.py.
The original one provided by Spacy is still there: scripts/parse_data.py. If you want to use that one, you need to have a training file with the same structure as assets:annotations.jsonl. The project.yml might need to be changed aswell.
